...
func (s *Service) TransitionWork(ctx context.Context, p domain.Principal, requestID, id string, to domain.WorkStatus, expected int64) (domain.WorkPackage, error) {
	...
	current, err := s.store.GetWorkPackage(ctx, p.Organization, id)
	if err != nil {
		return domain.WorkPackage{}, fmt.Errorf("read work package for transition: %w", err)
	}
	if current.Version != expected {
		return domain.WorkPackage{}, &domain.VersionConflict{Entity: "work package", ID: id, Version: expected}
	}
	work := current
	...
	if to == domain.WorkAccepted && p.Role != domain.RoleSupervisor {
		return domain.WorkPackage{}, domain.ErrForbidden
	}
	err = s.store.WithinTx(ctx, func(repo repository.MutationRepository) error {
		...
		if err := repo.PersistWorkPackage(ctx, work); err != nil {
			return err
		}
