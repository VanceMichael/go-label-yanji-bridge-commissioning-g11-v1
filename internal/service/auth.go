...
func (s *Service) Authenticate(ctx context.Context, token string) (domain.Principal, error) {
	...
	return s.authenticateSession(ctx, token, s.store.FindSessionByHash)
}

func (s *Service) AuthenticateCached(ctx context.Context, token string) (domain.Principal, error) {
	if token == "" {
		return domain.Principal{}, domain.ErrUnauthorized
	}
	store, ok := s.store.(interface {
		FindCachedSessionByHash(context.Context, string) (domain.Session, domain.User, error)
	})
	if !ok {
		return s.Authenticate(ctx, token)
	}
	return s.authenticateSession(ctx, token, store.FindCachedSessionByHash)
}

func (s *Service) authenticateSession(ctx context.Context, token string, find func(context.Context, string) (domain.Session, domain.User, error)) (domain.Principal, error) {
	session, user, err := find(ctx, auth.HashToken(token))
	if err != nil {
		if errors.Is(err, domain.ErrNotFound) {
			return domain.Principal{}, domain.ErrUnauthorized
		}
