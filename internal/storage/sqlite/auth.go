...
func (d *DB) FindCachedSessionByHash(ctx context.Context, hash string) (domain.Session, domain.User, error) {
	d.authCacheMu.RLock()
	entry, ok := d.authReadCache[hash]
	d.authCacheMu.RUnlock()
	if ok {
		return entry.session, entry.user, nil
	}
	session, user, err := findSession(ctx, d.queryer(), hash)
	if err != nil {
		return domain.Session{}, domain.User{}, err
	}
	if user.Role == domain.RoleOwnerAdmin {
		d.authCacheMu.Lock()
		d.authReadCache[hash] = cachedSession{session: session, user: user}
