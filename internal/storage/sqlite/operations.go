...
func (d *DB) EnqueueJobDetached(ctx context.Context, j domain.Job) error {
	return enqueueJob(ctx, d.queryer(), j)
}
func (t *txRepo) EnqueueJobDetached(ctx context.Context, j domain.Job) error {
	return enqueueJob(ctx, t.DB.queryer(), j)
}
...
func completeJob(ctx context.Context, q querier, id, owner string, now time.Time) error {
	result, err := q.ExecContext(ctx, `UPDATE jobs SET status='completed',lease_owner=NULL,lease_until=NULL,last_error='' WHERE id=? AND status='leased' AND lease_owner=? AND lease_until>?`, id, owner, timestamp(now))
	if err != nil {
		return fmt.Errorf("complete job: %w", err)
	}
	return requireAffected(result, "job", id, 0)
}
